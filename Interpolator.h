...
class Interpolator {
	public:
		...
		vector<double> samples; // pointer to the vector with sampled values
		double tMin, tMax; // times of first and last samples
		
		double deltaT; // sampling time interval
		const int sampleRange = 50; // range of samples to use for interpolation
		...
		void init(unsigned _nSamples,double _tMin, double _tMax, vector<double> &_samples){
